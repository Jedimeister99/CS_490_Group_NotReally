...
#import pywavefront
#scene=pywavefront.Wavefront('greg.obj')
def extractPointsFromObj(objFile):
	...
	with open(objFile,'r') as f:
		for line in f:
			...
			if len(line) <=0:
				...
				temp = [float(point) for point in line[4:7]]
				colors.append(temp)
			elif line [0] == 'f':
				temp = [int(int(point) -1) for point in reversed(line[1:4])]
				faces.append(temp)
	...
	return allPoints, faces, colors
vertices, triangles, colors = extractPointsFromObj('/home/pattig/code/face3d/examples/greg.obj')
#vertices = C['vertices']; 
#global colors
#global triangles
