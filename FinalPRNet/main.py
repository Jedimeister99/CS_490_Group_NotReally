            ...
            for seq in os.listdir(os.path.join(CKfilePath, subject)):
                if(os.path.isdir(os.path.join(CKfilePath, subject, seq))):
                    ...
                    for image in os.listdir(os.path.join(CKfilePath, subject, seq)):
                            types = ('*.jpg', '*.png')
                            save_folder = os.path.join("/home/pattig/code/PRNet/output", subject, seq)
                            if not os.path.exists(save_folder):
                                os.makedirs(save_folder, exist_ok=True)
                            # read image
                            if image.endswith ('.png'):
                                image_path = (os.path.join(CKfilePath, subject, seq, image))
                                print (image_path)
                                imagedata = imread(image_path)
                                ...
                                filename = image[:-4]
                                process_one(imagedata, args, prn, save_folder, filename)
def process_one(image, args, prn, save_folder, name):
